...
export class GraceStyleComponent extends KeywordComponentClass {
    ...
    constructor(protected olMapService: OlMapService, private modalService: BsModalService) {
        super(olMapService);
    }
    ...
    changeGraceStyle() {
        if (this.cswRecord !== null) {
            ...
            this.modalRef = this.modalService.show(StyleChooserModalComponent, { initialState });
            this.modalRef.content.onClose.subscribe(newStyle => {
                if (newStyle !== null) {
                    ...
                    const sld = GraceStyleService.getGraceSld(this.cswRecord.name, 'mascon_style', this.graceStyleSettings);
